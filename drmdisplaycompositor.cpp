...
int DrmDisplayCompositor::CommitFrame(DrmDisplayComposition *display_comp,
  ...
  //test tmp prev_layer
  /*
  std::vector<buffer_handle_t> tmp_prev_layers;
  tmp_prev_layers.clear();

  for (int i=0; i<prev_layers.size(); i++) {
      tmp_prev_layers.push_back(prev_layers[i]); 
  }
  prev_layers.clear();
  */
  ...
  for (DrmCompositionPlane &comp_plane : comp_planes) {
    ...
    if (comp_plane.type() != DrmCompositionPlane::Type::kDisable) {
      ...
      //final layer. This is the composited layer
      DrmHwcLayer &layer = layers[source_layers.front()];


      
      //change buffer test 
      //the original composited final layer is replaced by RGB test
      //handle layer hd

      
      //color experiment: print there sections of RGB respectively
      // cpu_addr   ARGB 0XAABBGGRR  
      // AA transparency
      // RGB: black: 000000 white: FFFFFF
      /*
      ...
      for (i = 0; i<layer.width*layer.height; i++){
          ALOGE("cpuaddr[%d] = %x",i,cpu_addr[i]);
          if(i < layer.width*layer.height/3){
              cpu_addr[i] = 0xFFFF0000;//blue
          }
          else if( i < layer.width*layer.height*2/3){
              cpu_addr[i] = 0xFF00FF00;//Green
          }
          else{
              cpu_addr[i] = 0xFF0000FF;//Red
          }
      ...
      unsigned char* cpu_addr_append = calloc(sizeof(unsigned char), layer.width*layer.height*2);
      int i, j;
      for( i = 0; i<layer.height; i++){
        for( j = 0; j < layer.width*2; j+=2){
          cpu_addr_append[i*layer.width*2 + j] = cpu_addr[i*layer.width + j];
          cpu_addr_append[i*layer.width*2 + j + 1] = cpu_addr[i*layer.width + j];
        }
      }

      &cpu_addr = &cpu_addr_append;
      gralloc_->unlock(gralloc_, layer.sf_handle);*/


      //display log
      ALOGE("strech display_ = %d",display_);
      ALOGE("strech crtc = %d",crtc->id());
      ALOGE("strech layer_handle = %d", layer.sf_handle);
      /*
      ALOGE("strech display_damage sf_layer = %d", layer.raw_sf_layer->surfaceDamage.numRects);
      ALOGE("strech comp_plane.type = %d", comp_plane.type());
      ALOGE("strech tmp_layers,size = %d", tmp_prev_layers.size());
      */
      ALOGE("strech struct.addr = %p", this);
  
      /*test of strech final version with a prev_layer_list
      //test check if layer.handle is in prev_layer
      int modified = 1;
      for (unsigned i=0; i<tmp_prev_layers.size(); i++){
          if(layer.sf_handle == tmp_prev_layers.at(i)){
            modified = 0;
          }
      }
      prev_layers.push_back(layer.sf_handle);
      

      if (modified == 0){
        ALOGE("strech have not modified layer.handle = %d", layer.sf_handle);
      }

      if (modified == 1){
        //strech the buffer in width
        int exchange_width = layer.width;
        int exchange_height = layer.height;
        int exchange_size = exchange_width * exchange_height;
        
        // ALOGE("SquashState::GenerateHistory expected %zu layers but got %zu layers", last_handles_.size(), num_layers);
        // Protected layers can't be squashed so we treat them as constantly
        // changing.
        //if (layer.protected_usage() || last_handles_[source_layers.front()] != layer.sf_handle){
        
      
        if (display_ == HWC_DISPLAY_PRIMARY &&  layer.raw_sf_layer->surfaceDamage.numRects!=0){
          unsigned int* cpu_addr;
          gralloc_->lock(gralloc_, layer.sf_handle, GRALLOC_USAGE_SW_READ_MASK | GRALLOC_USAGE_SW_WRITE_MASK, //gr_handle->usage,
                      0, 0, layer.width, layer.height, (void **)&cpu_addr);

          unsigned int* cpu_addr_exchange = (unsigned int*)calloc(exchange_size, sizeof(unsigned int));
          memcpy(cpu_addr_exchange, cpu_addr, exchange_size*sizeof(unsigned int));

          int i, j;
          for( i = 0; i<exchange_height; i++){
            for ( j = 0; j < exchange_width/2 ; j++){
              cpu_addr[i*exchange_width + j*2] = cpu_addr_exchange[i*exchange_width + j];
              cpu_addr[i*exchange_width + j*2 + 1] = cpu_addr_exchange[i*exchange_width + j];
            }
          }
          gralloc_->unlock(gralloc_, layer.sf_handle);
          free(cpu_addr_exchange);
        }
    ...
    int scale_factor;
    float src_bpp;
    //how much did the scr cutter shift
    int lower_shift = 0;
    int top_bar_flag = 0;
    ...
    ALOGE("strech: display_=%d Before modify: src_l=%d, src_t=%d, src_w=%d, src_h=%d", display_, src_l, src_t, src_w, src_h);

    //draw the upper display test, modify window manager
    //directly divide the finalized layer into two parts
    //if the resolution of HDMI streched, replug the cable
    

    //12.12 no status bar 64
    ...
    ALOGE("strech: display_=%d Before modify: dst_l=%d, dst_t=%d, dst_w=%d, dst_h=%d", display_, dst_l, dst_t, dst_w, dst_h);
    ...
    if(display_ == HWC_DISPLAY_PRIMARY ){
      ...
      if (dst_t >= 1080){
        ...
        src_h = 0;
        dst_h = 0;
        ALOGE("strech: do nothing so low"); 
      }
      //draw the #s in upper graph
      ...
      // start in upper but overlap to lower
        ...
        src_h = dst_t + dst_h - 1080;
        lower_shift = 1080 - dst_t;
        src_t += lower_shift;
        dst_t = 0;
        dst_h = src_h;
      }
    ...
    if(display_ == HWC_DISPLAY_PRIMARY ){
      ...
      if (fd >= 0) {
          ...
          ALOGE("semantic update write: %d %s", value, buffer);
          close(fd);
      }
      else
      {
      ...
      if (fd >= 0) {
          ...
          int bytes = sprintf(buffer, "%d\n", value);
          int amt = write(fd, buffer, bytes);
          ALOGE("semantic update write2: %d %s", value, buffer);
          close(fd);
      }
      else
      {
    ...
    if(display_ == HWC_DISPLAY_PRIMARY){
      ...
      if (top_bar_flag == 1){
        //+1080 have top bar on the under screen
        dst_t = src_t;
        //for safety
        dst_h = src_h;
      }
      else{
        //dst_t = dst_t + lower_shift - 1080;
        dst_h = src_h;
      }
    }*/

    ALOGE("strech: display_=%d After modify: src_l=%d, src_t=%d, src_w=%d, src_h=%d", display_, src_l, src_t, src_w, src_h);
    ALOGE("strech: display_=%d After modify: dst_l=%d, dst_t=%d, dst_w=%d, dst_h=%d", display_, dst_l, dst_t, dst_w, dst_h);

    /*
    //12.5 test
    ALOGE("strech: display_=%d, bClone=%d", display_, layers[source_layers.front()].bClone_);

    ALOGE("strech: Before modify: src_l=%d, src_t=%d, src_w=%d, src_h=%d", src_l, src_t, src_w, src_h);
    ...
    if(display_ == HWC_DISPLAY_PRIMARY){
      ...
      // start in upper but overlap to lower
        ...
        src_h = src_t + src_h - 1080;
        lower_shift = 1080 - src_t;
        src_t = 1080;
      }
    }
    

    ALOGE("strech: After modify: src_l=%d, src_t=%d, src_w=%d, src_h=%d", src_l, src_t, src_w, src_h);
    ...
    ALOGE("strech: Before modify: dst_l=%d, dst_t=%d, dst_w=%d, dst_h=%d", dst_l, dst_t, dst_w, dst_h);
    ...
    ALOGE("strech: After modify: dst_l=%d, dst_t=%d, dst_w=%d, dst_h=%d", dst_l, dst_t, dst_w, dst_h);*/
    ...
    //src_h = ((1080 - src_t) < src_h) ? (1080 - src_t) : src_h;
    ...
    //upper display test
    //change the dst_h and dst_t to match the display 1920x1080 
    //dst_h + dst_t = 1080 
    ...
    //dst_h = ((1080 - dst_t) < src_h) ? (1080 - dst_t) : src_h;

    //https://blog.csdn.net/lintao8613/article/details/79233485
    //logical display /frameworks/base/services/core/java/com/android/server/display/LogicalDisplay.java


    //current status: the navigation bar is override to the top
...
//zxl: src_l/src_w need 16 pixels aligned and src_t/src_h need 4 pixels aligned in FBDC area.
    ...
    if(DUAL){
      ...
      if (crtc->id() == 0){
          dst_w = dst_w;
          // ALOGD_IF(log_level(DBG_VERBOSE),"crtc id=%d",crtc->id());
      }
      else if (crtc->id() == 1){
        if(add == 1){
          dst_l += dst_w;
  ...
  //test: clear the vector of layers record
  //prev_layers.clear();
