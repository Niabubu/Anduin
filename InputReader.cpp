...
//12.20 test
//init global q_state
//Q_state_t *q_state = q_state->init();
...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    if (mParameters.hasAssociatedDisplay) {
        ...
        newViewport.setNonDisplayViewport(rawWidth, rawHeight); //1920x1080
    }
    bool viewportChanged = mViewport != newViewport;

    // set property of view port
    if (viewportChanged) {
        ...
        if (mDeviceMode == DEVICE_MODE_DIRECT || mDeviceMode == DEVICE_MODE_POINTER) {
            ...
            // mSurfaceWidth = naturalLogicalWidth;
            // mSurfaceHeight = naturalLogicalHeight;
            mSurfaceLeft = naturalPhysicalLeft * naturalLogicalWidth / naturalPhysicalWidth;
            mSurfaceTop = naturalPhysicalTop * naturalLogicalHeight / naturalPhysicalHeight;

            // device.identifier
            // usb-fe3c0000.usb-1.4/input0
            // usb-fe3c0000.usb-1.2/input0



            if(mSurfaceHeight == 2160 && strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string(), "usb-fe3c0000.usb-1.4/input0") == 0){
                ALOGE("eventhub usb = 1.4");
                //mSurfaceTop = 1080;
            }
            else if(mSurfaceHeight == 2160 && strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string(), "usb-fe3c0000.usb-1.2/input0") == 0){
                ALOGE("eventhub usb = 1.2");
    ...
    if (viewportChanged || deviceModeChanged) {
        ...
        //12.12 test
        //shift orientation not yet finished
        ...
        default:
            ...
            if(mSurfaceHeight == 2160){
                // split
                mOrientedRanges.y.min = mYTranslate + 1080;
...
void TouchInputMapper::dumpSurface(String8& dump) {
    //logical: 2160 physical: 2160 device: 1080
...
void TouchInputMapper::cookPointerData() {
    ...
    for (uint32_t i = 0; i < currentPointerCount; i++) {
        ...
        int logicalheight = mViewport.logicalBottom - mViewport.logicalTop;

        ALOGE("eventhub before orient mSurfaceheight = %d location = %s", logicalheight , 
        this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string());
        ALOGE("eventhub xTransf = %f yTransf = %f xMin = %f yMin = %f xTransl = %f yTransl = %f" , 
        ...
        //12.12 test for 1920x2160
        //12.13 after changing configure no need for that
        //1.4 primary
        //1.2 external
        ...
        ALOGE("eventhub second: %ld\n", tv.tv_sec); 
        ALOGE("eventhub millisecond: %ld\n", tv.tv_sec * 1000 + tv.tv_usec / 1000);
        ALOGE("eventhub microsecond: %ld\n", tv.tv_sec * 1000000 + tv.tv_usec);
        float shift = 1080;

        //surface height
        if(logicalheight == 2160 && strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string(), "usb-fe3c0000.usb-1.4/input0") == 0){
            y = y/2.0;
            ...
            if (fd >= 0) {
                ...
                ALOGE("semantic input write: %d %s", value, buffer);
                close(fd);
            }
            else
            {
        ...
        else if(logicalheight == 2160 && strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string(), "usb-fe3c0000.usb-1.2/input0") == 0){
            y = shift + y/2.0;
            ...
            if (fd >= 0) {
                ...
                int bytes = sprintf(buffer, "%d\n", value);
                int amt = write(fd, buffer, bytes);
                ALOGE("semantic input write2: %d %s", value, buffer);
                close(fd);
            }
            else
            {
        ...
        if(logicalheight == 1920 && strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string(), "usb-fe3c0000.usb-1.4/input0") == 0){
            x = shift + x/2.0;
            ALOGE("eventhub usb = 1.4 default");
            ALOGE("eventhub x = %f y = %f bottom = %f top = %f" , x, y, bottom, top);
            ALOGE("eventhub xTransf = %f yTransf = %f xMin = %f yMin = %f xTransl = %f yTransl = %f" , 
        ...
        else if(logicalheight == 1920 && strcmp(this->getEventHub()->getDeviceIdentifier(this->getDeviceId()).location.string(), "usb-fe3c0000.usb-1.2/input0") == 0){
            x = x/2.0;
            ALOGE("eventhub usb = 1.2 default");
            ALOGE("eventhub x = %f y = %f bottom = %f top = %f" , x, y, bottom, top);
            ALOGE("eventhub xTransf = %f yTransf = %f xMin = %f yMin = %f xTransl = %f yTransl = %f" , 
        ...
        //12.20 test
        //test 1.2
        //q_state->dump(q_state);
        //state_dump(q_state_p);
        /*
        int fd = open(HAL_LIBRARY_PATH2, O_RDWR|O_APPEND|O_CREAT, S_IRUSR | S_IRGRP | S_IROTH);
